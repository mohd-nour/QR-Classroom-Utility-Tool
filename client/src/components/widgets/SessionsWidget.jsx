...
var createSessionCardWrapped = function(courseData) {
  return function createSessionCard(session) {
    return (
      <SessionCard sessionNumber={session.sessionNumber} key = {session.sessionUniqueId} courseData={courseData}/>
    );
}
};
        ...
        <div>
          <div id="lower-section">
            ...
            {!sessions.length ? (
              ...
              <div id="card-section">{sessions.map(createSessionCardWrapped(props.data))}</div>
            )}
          </div>
        </div>
    ); 
